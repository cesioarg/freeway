...
class Freeway(object):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if not rulesfile:
            ...
            assert rulesfile and os.path.exists(
                rulesfile
    ...
    def info_from_path(self, firstMatch=True):
        ...
        """
        patterns = (
            list(self._rules.keys()) if self._pattern == ["auto"] else self._pattern
        )
                ...
                for field in rule.fields:
                    if field in rules:
                        rule.rule = rule.rule.replace(
                            "{%s}" % field, rules.get(field, field)[0].rule
                        )
            ...
            for rule in rules:
                try:
                    ...
                    if not ignoreMissing:
                        raise Exception(
                            "Can't find [%s] attribute(s)." % ", ".join(missing)
                        )
    ...
    def copy(self):
        return Freeway(
            ...
            rules=self._rules,
            convertionTable=self._convertionTable,
            **self.data
        )
